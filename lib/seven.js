...
export default class Seven extends EventEmitter{
  ...
  /**
   * @method _flushDb
   * @desc 12 hours to flush error dump
   * @private
   */

   _flushDb(){

      everytime('12 hour' || this.conf.flush, ()=>{
  ...
  /**
   * @method _cronJob
   * @desc cronjob execute
   * @private
   */
   _cronJob(){

     debug('cron started, waiting')
  ...
  _pushbulletMessage(error){
    ...
    (() =>{
      return new Promise((resolve,reject)=>{
        this._pushBullet.devices((perror, response) =>{
          if(perror){
            ...
            Pdebug(`pushbullet devices searching...`)
      ...
      var message = format(this.conf.pushbullet.message || `There was an error in your application, check your email address`,extrap);
          message = `${message}  -  ${error.name} `;

        this._pushBullet.note(device.iden, `Seven`, message,(perror,response)=>{
          Pdebug(`${device.nickname} send message `);
        });
  ...
  _slackMessage(error){
    ...
    var message = format(this.conf.slack.message || `There was an error in your application, check your email address`,extrap);
        message = `${message}  -  ${error.name} `;
  ...
  error(err,extra={}){
    if(!_.find(this._sevendb,{name:err.message})){
      ...
      if( typeof prettifyError(err) == 'undefined'){
        ...
        let params = {err,filename,xx,code,fakestack};
        let body = ejs.compile(fs.readFileSync(`${__dirname/error.tmp}`,'utf8'))(params);
  ...
  _template(error){
    ...
    if(!this.conf.template.title) {
      xeror.title = `Seven found new error : ${xeror.name}`;
    }
